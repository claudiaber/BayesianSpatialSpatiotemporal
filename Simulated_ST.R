...
print(arima(phi_t, order = c(1,0,0),include.mean = FALSE))
...
plots_ST[[1]] <- ggplot(data.frame(Chi_t= phi_t, Timestamp=1:n_tmstp),
                        aes(y=Chi_t, x=Timestamp)) +
  geom_line() + ylab(expression(chi[t]))
...
pdf(file = "./010_IJF_code_2/ProbabilityTheta_ST_20210228.pdf")
...
plots_ST[[2]] <- plots_ST[[2]] +
  theme(text = element_text(size=20))
pdf(file = "./010_IJF_code_2/ProbabilityTheta_ST_bigger.pdf")
do.call(gridExtra::grid.arrange, c(plots_ST,
                                   list(nrow=2,
                                        layout_matrix=matrix(c(1,2,2,2), ncol=1 ))
))
...
save(logit, phi, prob, W, x1_matrix, x2_matrix, Y_matrix, 
     ...
     file="./010_IJF_code_2/Simulated_data_models_ST.rdata")

# load("./010_IJF_code_2/Simulated_data_models_ST.rdata")
# model diagnostics ----------
library(rstan)
...
tableSummaryTex(summarytab = Scenario,round = 2,rownames = NULL,
                label = "tab:summaryST",
                caption = "prova",
                filename =  "./010_IJF_code_2/simulationstudy_ST.tex")
...
save(  Fitted,
       file="./010_IJF_code_2/Simulated_data_fitted_ST.rdata")
# load("./010_IJF_code_2/Simulated_data_fitted_ST.rdata")
...
tableSummaryTex(summarytab = PerfInsample,round = 2 ,rownames = rownames(PerfInsample) ,
                label = "tab:PerfInsampleST",
                caption = "prova",
                filename =  "./010_IJF_code_2/simulationstudy_ST_PerfInsample.tex")
...
tableSummaryTex(summarytab = Fore1$PerfOutTime,round = 2 ,rownames = rownames(Fore1$PerfOutTime) ,
                label = "tab:PerfOutTime_ST",
                caption = "prova",
                filename =  "./010_IJF_code_2/simulationstudy_ST_PerfOutTime.tex")
...
save(fitglm2, fitstan2, fitcarstan2, fitSTcar2, fitSTtrend2, 
     file="./010_IJF_code_2/Simulated_data_models_retrain_ST.rdata")
# load("./010_IJF_code_2/Simulated_data_models_retrain_ST.rdata")
...
rownames(PerfOutTime_multiple) <- c("AUC", "$REC\\_{0.03} (0-7.53)$",
                                    "$REC\\_{0.05} (0-12.55)$",
                                    "$REC\\_{0.1} (0-25.09)$" ,
                                    "$REC\\_{0.5} (0-100)$" , 
                                    "Recall", "Precision" )
...
tableSummaryTex(summarytab = PerfOutTime_multiple,round = 2 ,rownames = rownames(PerfOutTime_multiple) ,
                label = "tab:PerfOutTime_multiple_ST",
                caption = "prova",
                filename =  "./010_IJF_code_2/simulationstudy_ST_PerfOutTime_multiple.tex")
